/**
 * Zolto's first-party research — the pilot store's month-one numbers, as
 ...
 *   - Only numbers Zolto actually measured. No projections, no illustrative
...
export const PILOT_METHODOLOGY = {
  ...
  sample: `One pilot store — ${maker.founder ? `${maker.brand}, a pearl-jewelry maker in ${maker.city}` : `a pearl-jewelry maker in ${maker.city}`} — over the first 30 days after its Zolto storefront went live.`,
  collection:
    "Order and traffic figures are taken from the store's own Zolto dashboard over the period; offline sales are the merchant's own count. Nothing is modelled or extrapolated.",
  limits: [
    ...
    "Zolto operates the platform these numbers come from, so treat them as a vendor-published case, and weigh them accordingly.",
  ],
} as const;
