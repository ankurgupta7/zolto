...
export default function About() {
  ...
  return (
    <div className="page-enter pt-20">
        ...
        <div className="container">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-16 items-start">
              ...
              {/* Provenance sidebar */}
              <div className="lg:col-span-2 space-y-0 border-l border-[var(--brand-accent)]/30 pl-8">
                {[
                  ...
                  <div key={item.label} className="py-5 border-b border-[var(--brand-border)] last:border-0">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-[var(--brand-accent)] font-sans mb-1">
                      {item.label}
                    </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {VALUES.map((v) => (
              <div key={v.titleKey} className="text-center">
                <div className="text-4xl text-[var(--brand-accent)] font-serif mb-4">{v.icon}</div>
      ...
      {/* Materials */}
      <section className="py-24 bg-[var(--brand-ink)]">
        <div className="container">
          <div className="text-center mb-16">
            <p className="text-[var(--brand-accent)] text-xs uppercase tracking-[0.3em] mb-3 font-sans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {MATERIALS.map((m) => (
              <div key={m.nameKey} className="border border-white/10 p-8 text-center">
                <h3 className="font-serif text-[var(--brand-accent)] text-xl mb-4">{t(m.nameKey)}</h3>
        ...
        <div className="container">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center">
              <div className="flex items-center gap-8 p-8 border border-[var(--brand-border)] bg-[var(--brand-surface-2)] max-w-sm w-full">
                {/* Monogram avatar */}
                <div className="w-16 h-16 flex-shrink-0 bg-[var(--brand-ink)] flex items-center justify-center">
                  <span className="font-serif text-[var(--brand-accent)] text-2xl">SA</span>
                </div>
                <div>
                  <p className="font-serif text-foreground text-xl">{t("about.teamCeoName")}</p>
                  <p className="text-[var(--brand-accent)] text-xs uppercase tracking-[0.15em] font-sans mt-1">
                    {t("about.teamCeoRole")}
                  </p>
      ...
      {/* CTA */}
      <section className="py-20 bg-[var(--brand-surface)] text-center">
        <div className="container">
          ...
          <Link
            href="/shop"
            className="inline-flex items-center gap-2 bg-[var(--brand-ink)] text-white px-10 py-4 text-sm uppercase tracking-[0.15em] font-sans hover:bg-[var(--brand-ink-hover)] transition-colors duration-200"
          >
            {t("about.shopNow")}
          </Link>
        </div>
