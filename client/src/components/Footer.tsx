    ...
    <footer className="bg-[var(--brand-ink)] text-white/70">
      ...
      <div className="container py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2 mb-6">
              {COLLECTION_LINKS.filter((cat) => availableCategories.has(cat.name as ProductCategory)).map((cat) => (
                <li key={cat.name}>
                  <Link
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-white/30">
          <p>{t("footer.copyright", { year: new Date().getFullYear(), store: branding.storeName })}</p>
