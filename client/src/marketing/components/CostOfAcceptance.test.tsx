...
describe("CostOfAcceptance", () => {
  ...
  it("renders Gwinn's card row last, because that is where it lands", () => {
    ...
    // Swiss cards bill at the non-EEA rate, taking a card through Gwinn is the
    // dearest in-person option on the page.
    render(<CostOfAcceptance channel="in-person" />);
    const ids = screen
      .getAllByTestId(/^cost-row-/)
      .map((r) => r.dataset.testid!.replace("cost-row-", ""));
    expect(ids.at(-1)).toBe("platform-card");
    // …and TWINT sits second, which is the argument that survives.
    expect(ids[1]).toBe("platform-twint-qr");
  });

  it("prints the review's worked figures", () => {
    render(<CostOfAcceptance channel="in-person" />);
    const twint = within(screen.getByTestId("cost-row-platform-twint-qr"));
  ...
  it("says on the card row that Gwinn adds nothing to Stripe's rate", () => {
    // Being the dearest row is survivable; being the dearest row without
    // saying whose fee it is would not be.
    render(<CostOfAcceptance channel="in-person" />);
    const row = screen.getByTestId("cost-row-platform-card").textContent!;
  ...
  it("can drop the framing paragraphs a second table would repeat verbatim", () => {
    ...
    expect(screen.queryByText(/Gwinn is not at the top/i)).toBeNull();
  });

  it("shows both framing paragraphs by default", () => {
    render(<CostOfAcceptance channel="online" />);
    expect(screen.getByTestId("cost-of-acceptance-note")).toBeTruthy();
    expect(screen.getByText(/Gwinn is not at the top/i)).toBeTruthy();
  });
  ...
  it("shows only Gwinn and the named competitor on a compare page", () => {
    // Caught by screenshot: /compare/gwinn-vs-worldline was listing SumUp's
    ...
    expect(ids).toContain("platform-twint-qr");
  ...
  it("filters by channel", () => {
    render(<CostOfAcceptance channel="online" />);
    expect(screen.queryByTestId("cost-row-platform-twint-qr")).toBeNull();
    expect(screen.getByTestId("cost-row-sumup-online")).toBeTruthy();
  });

  it("puts SumUp above both Gwinn plans in the online table", () => {
    render(<CostOfAcceptance channel="online" />);
    const ids = screen
      .getAllByTestId(/^cost-row-/)
      .map((r) => r.dataset.testid!.replace("cost-row-", ""));
  ...
  it("renders money in lining figures", () => {
    ...
    const { container } = render(<CostOfAcceptance channel="in-person" />);
    const cost = within(
      screen.getByTestId("cost-row-platform-twint-qr"),
    ).getByText("CHF 0.59");
...
describe("CostOfAcceptance — the intro states the finding, not a softer version", () => {
  it("says every other option beats us on card rate", () => {
    ...
    // while Stripe's Swiss-card bucket was unknown and the Gwinn card row sat
