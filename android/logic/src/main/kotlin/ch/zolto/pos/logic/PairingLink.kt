...
/**
 * One-tap pairing links: `zolto://pair?t=<token>&url=https://their-store.zolto.ch`
 *
 ...
 * in step with the iOS counterpart (ios/ZoltoPOS/ZoltoPOS/Logic/Pairing.swift),
 * which the two test suites pin against the same link forms.
 */
object PairingLink {

    /** Fallback when a link carries no server of its own. */
    const val DEFAULT_BASE_URL = "https://zolto.ch"

    const val SCHEME = "zolto"
     ...
     * delivers matter: `zolto://pair?…` (action in the authority) and
     * `zolto:/pair?…` (action in the path).
     */
    fun parse(link: String?): Parsed? {
        ...
        // Everything after "zolto:", with any leading slashes removed, is
