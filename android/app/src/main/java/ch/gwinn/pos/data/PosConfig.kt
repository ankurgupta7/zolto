...
/**
 * Runtime POS configuration — which gwinn deployment + which store (tenant)
 * this register belongs to. Unlike the single-tenant build (which baked one
 * store's URL + key into the APK via secrets.xml), gwinn is multi-tenant:
...
object PosConfig {
    ...
    private const val PREFS = "gwinn_pos_config"
