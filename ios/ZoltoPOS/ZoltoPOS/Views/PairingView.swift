...
/// First-run pairing: connect this device to a Zolto store with its POS API
...
struct PairingView: View {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        Button {
                            ...
                            if isConnecting {
                                ProgressView().tint(.zoltoBackground)
                            } else {
                                Text("Connect")
                            }
                        }
    ...
    private func handleScan(_ payload: String) {
        guard let credentials = Pairing.parseQrPayload(payload) else {
            errorMessage = "That QR code doesn't look like a Zolto POS pairing code."
            return
        }
