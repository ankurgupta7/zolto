# Content Release Form

## Kalakosh × Zolto Content Partnership

**Date:** _______________

**Content Creator:** Kalakosh ("Creator")
**Business Address:** _________________________________________________
**Contact Email:** _________________________________________________

**Content Publisher:** Zolto ("Publisher")
**Business Address:** _________________________________________________
**Contact Email:** _________________________________________________

---

## 1. Purpose

Kalakosh is the first customer (Tenant #1) on the Zolto platform. Zolto would like to document and publish Kalakosh's journey of launching their first online store, including but not limited to:

- Blog posts and articles
- Photographs of workspace, products, and process
- Video recordings and interviews
- Screenshots of the store and admin dashboard
- Metrics and business data (sales numbers, order volume, etc.)
- Quotes and testimonials

---

## 2. Grant of Rights

Creator hereby grants Publisher a perpetual, irrevocable, royalty-free, worldwide license to:

### 2.1 Content Uses
- [ ] **Photographs** — Publish photos of Creator's workspace, products, and process
- [ ] **Video** — Publish video recordings of Creator and their work
- [ ] **Screenshots** — Publish screenshots of Creator's store and admin dashboard (with sensitive data redacted)
- [ ] **Metrics** — Publish anonymized or attributed business metrics (sales, orders, growth)
- [ ] **Quotes** — Use direct quotes from Creator in marketing materials
- [ ] **Name and Brand** — Use Creator's business name ("Kalakosh") in case studies and marketing

### 2.2 Distribution Channels
- [ ] Zolto website and blog
- [ ] Zolto social media accounts
- [ ] Press releases and media outreach
- [ ] Investor and advisor presentations
- [ ] Email newsletters

---

## 3. Creator Rights and Protections

### 3.1 Approval Rights
Creator has the right to review and approve any content before publication. Publisher will submit drafts within **48 hours** of completion.

### 3.2 Sensitive Data
Publisher agrees to:
- Redact customer personal information (names, addresses, emails)
- Redact financial account details (Stripe IDs, exact revenue if requested)
- Not publish proprietary business information without consent

### 3.3 Attribution
Creator may request:
- [ ] Full attribution ("Kalakosh, jewelry maker in Zurich")
- [ ] Partial attribution ("A jewelry maker in Switzerland")
- [ ] Anonymous ("A Zolto customer")

### 3.4 Revocation
Creator may revoke this authorization with **30 days written notice**. Previously published content may remain live but will not be used in new materials.

---

## 4. Compensation

Creator acknowledges that this content partnership is part of Zolto's free service tier. No monetary compensation is provided for content use.

**Benefits to Creator:**
- Free use of Zolto platform
- Exposure to Zolto's audience
- Featured placement on Zolto's website and channels

---

## 5. Representations and Warranties

Creator represents that:
- They have the right to grant these licenses
- The content does not infringe third-party rights
- They are of legal age and authorized to sign

---

## 6. Signatures

**Creator (Kalakosh):**

Signature: _________________________

Name (printed): _________________________

Date: _________________________

---

**Publisher (Zolto):**

Signature: _________________________

Name (printed): _________________________

Title: _________________________

Date: _________________________

---

## 7. Content Inventory (Attach Separately)

Please attach:
- [ ] List of photographs provided
- [ ] List of video recordings provided
- [ ] Screenshots approved for use
- [ ] Specific metrics approved for publication

---

**Questions? Contact:** [partnerships@zolto.com]
