...
- **Same isolation pass is still owed on the other tenant-owned tables.** The product path is fixed; these read helpers still ignore `tenant_id` and need the same treatment: `orders` (`getPaidOrders`, `getOrderBySessionId`, `updateOrderBySessionId`), `instagram_posts` (`getInstagramPosts`), `pos_orders` (POS sales list / receipts), `bulk_upload_logs` (`getBulkUploadLogs`). Also the Stripe **reconciliation** job scans one Stripe account globally and currently matches against the default tenant — per-tenant Stripe reconciliation needs per-tenant Stripe accounts first.
