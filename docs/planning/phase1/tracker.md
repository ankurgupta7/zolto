...
| Task                                                                                     | Status                | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [ ] Upload Kalakosh products to online store                                             | 🤖 tool ready         | `scripts/import-kalakosh-live-catalog.ts` migrates the Kalakosh catalogue into this tenant, re-hosting photos and skipping anything already imported. 👤 Run it with the production `DATABASE_URL` + S3 creds — I don't have those in this sandbox. Set `KALAKOSH_DATABASE_URL` to the old deployment's database to get the **whole** inventory (hidden, sold-out, unphotographed pieces and gallery images); without it the script falls back to the public `products.list` API, which is the storefront view and shows only the 2 visible, photographed products. `--dry-run` reports what it would import first. |
...
| [x] Set up order notifications                                                           | ✅                    | Customer receipt email already sends on payment (`sendOrderReceipt` in `_core/email.ts`, via Resend — `server/stripe.ts` `fulfillOrder`). Owner/tenant alert on new order already fires too (`notifyOwner` in `_core/notification.ts`), as a Discord DM to whichever account the deployment's `DISCORD_OWNER_USER_ID` is configured for — not hardcoded to any one person. There's no outbound WhatsApp channel (WhatsApp integration today is inbound-only, for product intake), so if a WhatsApp alert specifically is still wanted, that's the one open piece. |
...
### Added on branch `claude/work-progress-review-4j7s7n` (2026-07-31)

Closing out the pricing-pivot backlog. Everything below is unit-tested; the
"verified how" column says what that verification actually reached, because
several of these touch money or tenancy.

| Plan item                                    | Status in code | Verified how                                                                                                                                                                                                                                                                             |
| -------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stripe platform fee on online/agent orders   | ✅ Implemented | **Against real Stripe test mode** — `pnpm test:integration`, 16/16 green. This was the pivot's last blocker.                                                                                                                                                                              |
| Plan storage caps                            | ✅ Implemented | `server/storage.ts` — enforced at the `storagePut` choke point against `PLANS[].storageGb`, counted in `tenants.storage_bytes_used` (migration `0010`). Fails **open** if the tenant row is missing, so a lookup blip can't block uploads.                                                 |
| Shared/persistent rate limiter               | ✅ Implemented | `server/rateLimit.ts` — `RateLimitStore` interface with in-memory and DB stores (migration `0011`), double-guarded fail-open so a limiter outage never takes the app down.                                                                                                                |
| `tenantAdminProcedure` + cross-tenant sweep  | ✅ Implemented | `server/_core/trpc.ts`. Closed an **unauthenticated tenant-settings takeover** (`tenant.updateSettings` shipped as `publicProcedure`) plus the systemic `adminProcedure` + host-derived-`ctx.tenant` gap. Every converted route has a test that an admin of a *different* tenant is refused. |
| TWINT QR-sticker POS rail                    | ✅ Implemented | Merchant uploads their own TWINT QR; POS displays it; the sale is recorded as an attested method. `server/pos.ts` (`ATTESTED_METHODS` allow-list), admin UI, Android (migration `0013`). **Kotlin now compiler-verified in CI** — see below.                                              |
| Per-tenant Stripe reconciliation             | ✅ Implemented | `server/reconciliation.ts` runs on each tenant's **connected account** (`{ stripeAccount }`), matches only that tenant's catalogue, writes an explicit `tenantId`. Plus `platform.reconcileAllTenants` (`superadminProcedure`) for the platform side.                                     |
