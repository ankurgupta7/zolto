...
**Goal:** let a small business move to Gwinn from whatever they're on today with
as little friction as possible — ideally a guided "import your store" step in
onboarding. The whole premise of Gwinn is that incumbents overcharge small
...
- **Generic CSV** — a well-documented column mapping as the universal fallback
  (Gwinn already has a CSV importer for products; extend it into a first-class,
...
- Per-source auth: OAuth app (Shopify/Square) vs API key (Stripe) vs pure CSV
  ...
  most makers fastest.
- Image handling: re-host imported product images into Gwinn's S3 (imported URLs
  will rot / be access-controlled on the source).
- De-dupe on re-import (Gwinn's CSV importer already matches by name — reuse
  that).
...
**Goal:** make Gwinn and every storefront legible and usable by LLMs and AI
...
- **`/llms.txt` + `/llms-full.txt`** (llmstxt.org format), tenant-aware: the
  platform apex serves a rich Gwinn brief (features, pricing, how-to-start, MCP
...
- **Surface-aware MCP** at `POST /mcp` (`server/mcp.ts`):
  ...
  - _Platform / marketing_ (no tenant, e.g. gwinn.com): `get_platform_info`,
