# Gwinn — Planning & Execution Docs

Business strategy, Phase 1 execution kit, and the code roadmap for turning Gwinn
...
| Path | What it is |
|------|-----------|
| [`gwinn-business-plan.md`](./gwinn-business-plan.md) | The business plan (v1.1). Executive summary, market, pricing, financials, risks, phase roadmap. Start here. |
...
| [`native-twint-integration.md`](./native-twint-integration.md) | **Research note on moving in-person payments to native TWINT.** What registration and approval TWINT requires (Gwinn counts as an *integrator* and must be certified), the Store UUID + `.p12` certificate credential model, how it maps onto `tenantSecrets`/`pos.ts`, and the one measurement to take before committing engineering time. |
| [`bring-your-own-payments.md`](./bring-your-own-payments.md) | **Research note on letting merchants bring their own payment provider** (Stripe / Payrexx / SumUp / Worldline) and positioning Gwinn as a front end to the rails. Finds that BYO-Stripe already works, that the 1% breaks on providers without a commission API — so BYO should gate to Pro — and that in-person should stay on one rail because the own-TWINT-QR path is already provider-independent. |
...
| [`ai-traffic-alignment.md`](./ai-traffic-alignment.md) | **Live plan.** Gap register + phased work to align Gwinn's marketing surface and tenant storefronts with the WebFX 600K-AI-session study. Tracks progress with checkboxes. |
...
1. **Content release is required.** Publishing Kalakosh's name, photos, and story to market
   Gwinn needs a signed content/publicity release — Kalakosh being Sheena Arora's business does
   *not* remove that need (Gwinn is a separate party). See `gwinn-business-plan.md` §5.1 and
...
2. **Frontend SaaS pages are on hold.** The signup/onboarding/pricing/legal pages are drafted
   but not built, because the `gwinn` client is still a fork of the Kalakosh storefront. They're
...
- `gwinn-business-plan-ai-readable.json` — it was a machine-readable mirror of the **v1.0** plan and
...
_Business plan v1.1 revision notes are at the top of `gwinn-business-plan.md`._
