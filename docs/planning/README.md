...
| [`bring-your-own-payments.md`](./bring-your-own-payments.md) | **Research note on letting merchants bring their own payment provider** (Stripe / Payrexx / SumUp / Worldline) and positioning Zolto as a front end to the rails. Finds that BYO-Stripe already works, that the 1% breaks on providers without a commission API — so BYO should gate to Pro — and that in-person should stay on one rail because the own-TWINT-QR path is already provider-independent. |
