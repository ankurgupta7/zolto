...
## Case A — fresh, standalone Gwinn (recommended first)

Gwinn runs on its **own** server + **own** (empty or near-empty) database, onboarding
...
```bash
# on the Gwinn server, in the Gwinn repo, with .env configured:
bash update.sh
```

`update.sh` applies migrations 0000–0019 in order. 0019 creates the tenant tables
and seeds tenant #1 as `platform` / "Gwinn Platform" (override with
...
Only when you're deliberately moving a live store (e.g. Kalakosh) onto Gwinn. This
...
window, backup). The Gwinn server code must be deployed at the same time — a
...
Only after the dry-run passes: back up (`deploy/backup.sh`), deploy the Gwinn code
