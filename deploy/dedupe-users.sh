...
inspect() {
  ...
  if [ "${real:-0}" -lt 2 ]; then
    ...
    echo "customer's email as text, not a user id."
    echo ""
    echo "Whether it comes back depends on the deployed version. Since the magic-link"
    echo "callback learned to adopt an existing account (server/_core/magicLink.ts),"
    echo "a link for an address that manages exactly ONE store signs in as that"
    echo "account and creates no second row. On an older build, or for an address"
    echo "managing several stores — where adopting would have to guess which — the"
    echo "next magic-link sign-in parks a fresh row here again."
