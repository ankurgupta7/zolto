...
export function registerReconciliationRoutes(app: Express): void {
  ...
  app.get("/api/reconciliation/confirm", async (req: Request, res: Response) => {
    try {
      ...
      const candidateIds = parseCandidateIds(reconciliation.candidateProductIds);
      const rawChoice = typeof req.query.choice === "string" ? req.query.choice : "";
  ...
  app.post("/api/reconciliation/confirm", async (req: Request, res: Response) => {
    try {
